        ...
        <section>
            <table class="w-full text-sm text-left rtl:text-right">
                <thead class="text-xs text-white uppercase bg-gray">
                    <th scope="col" class="md:px-6 px-2 py-3">Orden</th>
                    <th scope="col" class="md:px-6 px-2 py-3 hidden md:table-cell">Estado</th>
                    <th scope="col" class="md:px-6 px-2 py-3">Seguimiento</th>
                    <th scope="col" class="md:px-6 px-2 py-3">Precio</th>
                    <th scope="col" class="md:px-6 px-2 py-3">Ver Pedido</th>
                </thead>
                <tbody>
                    @foreach ($orders as $order)
                        <tr class="border-y border-gray">
                            <td class="py-2">{{ $order->order_number }}</td>
                            <td class="py-2 hidden md:table-cell">{{ $order->status }}</td>
                            <td class="py-2 text-center"><a href="{{ $order->track_link }}"
                                    class="bg-red px-2 py-1 text-xs text-white rounded-sm">Seguir</a>
                            </td>
                            <td class="py-2">${{ number_format($order->price, 2, ',', '.') }}</td>
                            <td class="py-2 text-center"><a href="{{ route('order', $order->id) }}"
